  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 255, 255, 0.53);borderRadius:10px">
        <h1 class="h1" style="color:rgba(101, 126, 253, 1);fontSize:25px;">线上学习网站注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<el-form-item label="职位" class="input" v-if="tableName=='jiaoshi'">
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.shenfenzheng) && `xuesheng` == this.tableName){
      ...
      if((!this.ruleForm.shenfenzheng) && `jiaoshi` == this.tableName){
        this.$message.error(`身份证不能为空`);
        return
      }
